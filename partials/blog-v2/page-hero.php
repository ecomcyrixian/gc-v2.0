...
if ( function_exists( 'get_sub_field' ) ) {
    ...
    if ( is_string( $hero_title ) ) {
        $hero_title = trim( $hero_title );
    }
}

if ( empty( $hero_title ) || ( is_string( $hero_title ) && trim( strip_tags( $hero_title ) ) === '' ) ) {
    $hero_title = '<h1>' . esc_html( get_the_title() ) . '</h1>';
}
...
if ( function_exists( 'get_sub_field' ) ) {
    ...
    if ( $bg_image && ! empty( $bg_image ) ) {
        if ( is_string( $bg_image ) ) {
            ...
            if ( ! empty( $bg_image ) && ( filter_var( $bg_image, FILTER_VALIDATE_URL ) || strpos( $bg_image, '/' ) === 0 || strpos( $bg_image, 'http' ) === 0 ) ) {
                $hero_bg_image_url = $bg_image;
            }
        }
        elseif ( is_array( $bg_image ) ) {
            if ( isset( $bg_image['url'] ) && ! empty( $bg_image['url'] ) ) {
                $hero_bg_image_url = $bg_image['url'];
            } elseif ( isset( $bg_image['ID'] ) && is_numeric( $bg_image['ID'] ) ) {
                $hero_bg_image_url = wp_get_attachment_image_url( $bg_image['ID'], 'full' );
            }
        }
        elseif ( is_numeric( $bg_image ) ) {
            $hero_bg_image_url = wp_get_attachment_image_url( $bg_image, 'full' );
        }
    }
}
...
$hero_image_url = null;
if ( function_exists( 'get_sub_field' ) ) {
    $select_image = get_sub_field( 'select_image' );
    if ( $select_image && ! empty( $select_image ) ) {
        if ( is_string( $select_image ) ) {
            $select_image = trim( $select_image );
            if ( ! empty( $select_image ) && ( filter_var( $select_image, FILTER_VALIDATE_URL ) || strpos( $select_image, '/' ) === 0 || strpos( $select_image, 'http' ) === 0 ) ) {
                $hero_image_url = $select_image;
            }
        }
        elseif ( is_array( $select_image ) ) {
            if ( isset( $select_image['url'] ) && ! empty( $select_image['url'] ) ) {
                $hero_image_url = $select_image['url'];
            } elseif ( isset( $select_image['ID'] ) && is_numeric( $select_image['ID'] ) ) {
                $hero_image_url = wp_get_attachment_image_url( $select_image['ID'], 'full' );
            }
        }
        elseif ( is_numeric( $select_image ) ) {
            $hero_image_url = wp_get_attachment_image_url( $select_image, 'full' );
        }
    }
}

if ( ( ! $hero_image_url || empty( $hero_image_url ) ) && has_post_thumbnail() ) {
...
<header class="blog-v2-hero<?php echo ! $hero_image_url ? ' blog-v2-hero--no-image' : ''; ?>"<?php if ( $hero_bg_image_url ) : ?> data-bg-image="<?php echo esc_url( $hero_bg_image_url ); ?>" style="--bg-image: url('<?php echo esc_url( $hero_bg_image_url ); ?>');"<?php endif; ?>>
    ...
    <?php if ( $hero_image_url ) : ?>
        <div class="blog-v2-hero__media">
            <img src="<?php echo esc_url( $hero_image_url ); ?>" alt="<?php echo esc_attr( strip_tags( $hero_title ) ?: get_the_title() ); ?>" class="blog-v2-hero__image" />
        </div>
    <?php elseif ( has_post_thumbnail() ) : ?>
        <div class="blog-v2-hero__media">
            <?php the_post_thumbnail( 'full', array( 'class' => 'blog-v2-hero__image' ) ); ?>
        </div>
    <?php endif; ?>
</header>
