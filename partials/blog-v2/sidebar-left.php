    ...
    if ( $bg_image && ! empty( $bg_image ) ) {
        if ( is_string( $bg_image ) ) {
            ...
            if ( ! empty( $bg_image ) && ( filter_var( $bg_image, FILTER_VALIDATE_URL ) || strpos( $bg_image, '/' ) === 0 || strpos( $bg_image, 'http' ) === 0 ) ) {
                $sidebar_bg_image_url = $bg_image;
            }
        }
        elseif ( is_array( $bg_image ) ) {
            if ( isset( $bg_image['url'] ) && ! empty( $bg_image['url'] ) ) {
                $sidebar_bg_image_url = $bg_image['url'];
            } elseif ( isset( $bg_image['ID'] ) && is_numeric( $bg_image['ID'] ) ) {
                $sidebar_bg_image_url = wp_get_attachment_image_url( $bg_image['ID'], 'full' );
            }
        }
        elseif ( is_numeric( $bg_image ) ) {
            $sidebar_bg_image_url = wp_get_attachment_image_url( $bg_image, 'full' );
        }
    }
}
?>
<aside class="blog-v2-sidebar blog-v2-sidebar__container">
    <div class="blog-v2-sidebar--left">
        <div class="blog-v2-sidebar__toc-section">
            <h2 class="blog-v2-sidebar__heading">
            ...
            <div class="blog-v2-sidebar__divider"></div>
            <nav class="blog-v2-toc" aria-label="Table of Contents">
    ...
    if ( function_exists( 'have_rows' ) && have_rows( 'blog_v2_layout' ) ) :
        while ( have_rows( 'blog_v2_layout' ) ) : the_row();
            if ( get_row_layout() == 'card' ) :
                set_query_var( 'sidebar_position', 'left' );
