...
if ( $ppma_author && method_exists( $ppma_author, 'get_meta' ) ) {
    ...
    if ( $user_description === '' && method_exists( $ppma_author, 'get_user_object' ) ) {
        $u = $ppma_author->get_user_object();
        if ( $u instanceof WP_User ) {
            $user_description = (string) $u->description;
        }
    }
    $avatar_url = '';
    if ( method_exists( $ppma_author, 'get_avatar_url' ) ) {
    ...
    if ( $avatar_url === '' && $user_email !== '' ) {
        $avatar_url = get_avatar_url( $user_email, array( 'size' => 156 ) );
    }
    if ( $avatar_url === '' && method_exists( $ppma_author, 'get_user_object' ) ) {
        $u = $ppma_author->get_user_object();
        if ( $u instanceof WP_User ) {
            $ad = get_avatar_data( $u->ID, array( 'size' => 156 ) );
            if ( is_array( $ad ) && ! empty( $ad['url'] ) ) {
                $avatar_url = (string) $ad['url'];
            }
        }
    }
    if ( $avatar_url === '' && $queried instanceof WP_Term ) {
        $att_id = (int) get_term_meta( (int) $queried->term_id, 'avatar', true );
        if ( $att_id > 0 ) {
            $from_att = wp_get_attachment_image_url( $att_id, array( 156, 156 ) );
            if ( ! $from_att ) {
                $from_att = wp_get_attachment_image_url( $att_id, 'medium_large' );
            }
            if ( ! $from_att ) {
                $from_att = wp_get_attachment_image_url( $att_id, 'full' );
            }
            if ( $from_att ) {
                $avatar_url = $from_att;
            }
    ...
    // Same data as get_avatar(); local-avatar plugins hook pre_get_avatar_data.
    $ad               = get_avatar_data(
        get_queried_object_id(),
        array(
            'size' => 156,
            'alt'  => trim( $first_name . ' ' . $last_name ),
        )
    );
    $avatar_url       = ( is_array( $ad ) && ! empty( $ad['url'] ) ) ? (string) $ad['url'] : '';
    if ( $avatar_url === '' && $user_email !== '' ) {
        $ad = get_avatar_data( $user_email, array( 'size' => 156 ) );
        if ( is_array( $ad ) && ! empty( $ad['url'] ) ) {
            $avatar_url = (string) $ad['url'];
        }
    }
        ...
        if ( is_object( $pp_for_slug ) && isset( $pp_for_slug->term_id ) ) {
            ...
            if ( $has_pp ) {
                ...
                if ( $pp_img === '' && $has_att ) {
                    ...
                    if ( $aid > 0 ) {
                        $pp_img = wp_get_attachment_image_url( $aid, array( 156, 156 ) );
                        if ( ! $pp_img ) {
                            $pp_img = wp_get_attachment_image_url( $aid, 'full' );
                        }
                    }
                }
                if ( $pp_img !== '' ) {
                 ...
                 <div class="author-desc">
                    <div class="name-socmed">
                        ...
                        <div class="author-socmed">
                            <?php if ( ! empty( trim( $user_facebook ) ) ) : ?>
                                <a class="ppma-author-facebook-profile-data ppma-author-field-meta ppma-author-field-type-url" aria-label="Facebook" href="<?php echo esc_url( $user_facebook ); ?>"  target="_blank" rel="noopener noreferrer">
                                    <span>
                    ...
                    <div>
                        <?php echo nl2br( esc_html( $user_description ) ); ?>
                    </div>
                ...
                <div class="heading">
                    <h2>
                        <span>FROM THE AUTHOR</span>
                            More Articles by <?php echo esc_html( $display_name ); ?>
                    </h2>
                </div>
                <div class="cards">
